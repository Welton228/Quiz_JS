...
//funcao que retorna quantidade de perguntas aleatorias
const selecionarPerguntasAleatorias = (colecaoPerguntas, quantidade) => {
    const perguntasAleatorias = colecaoPerguntas.sort(() => Math.random() - 0.5);
    return perguntasAleatorias.slice(0, quantidade);
}

//funcao que exibe as pergunta e retorna a resposta do usuario
...
const validaResposta = (respostaDoUsuario, pergunta) => {
    return respostaDoUsuario === pergunta.resposta;
}

//funcao que calcula a pontuacao do usuario
const pontuacaoUsuario = (pontuacaoFinal, nomeDoJogador) => {
    ...
    console.log(`Jogador(a) : ${nomeDoJogador}`);
...
const queOsJogosComecem = () => {
    ...
    const quantidadeDeQuestoes = 10;

    const questoesSelecionadas = selecionarPerguntasAleatorias(questoes, quantidadeDeQuestoes);

    let pontuacaoFinal = 0;

    questoesSelecionadas.forEach((pergunta, index) => {
        const respostaUsuario = exibirPerguntas(pergunta, index);
        const resultadoValidacao = validaResposta(respostaUsuario, pergunta);
    ...
    pontuacaoUsuario(pontuacaoFinal, nomeJogador);
